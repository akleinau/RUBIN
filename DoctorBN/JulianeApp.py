...
#add all evidences of a file to the network and update beliefs
...
#returns a node object, combining the information about a node in net and net_old
...
#calculate and return the states + their probabilities of a node
...
#returnes the differences in probability between each variable of the two states
def compute_relevancies_for_outcome_states(state_1, state_2):
    all_rel_objects_for_current_node = []

    point_obj = classes.PointObject(index=0, relevance=0, x=state_1[0].name) #weird bc not used!

    for i in range(len(state_1)):

        prob_diff = float(state_2[i].probability) - float(state_1[i].probability) #compute prob diff of each state
        ...
        if prob_diff > point_obj.relevance: #find state with biggest difference. Weird bc not used!
            point_obj.index = i
...
#returns var with biggest difference in probability
...
def get_influence_of_evidences_on_target():
    ...
    #     get probabilites of all nodes with final evidences
    ...
    for i in range(len(target_ids)):
        ...
        for evidence_node in node_list_with_evidences:
            ...
            rel_of_ev_obj.overall_relevance = jensen_shannon_value #global relevance
            ...
            max_state_all_evidences = "" #most probable state
            max_prob_all_evidences = -1
            max_state_current_evidences = "" #most probable state
    ...
    #calculate percentage relevance for each evidence object on overall relevance
...
def get_all_observable_nodes():
    ...
    for node_handle in net_all.get_all_nodes():
        if int(net_all.get_node_diag_type(node_handle)) > 0:  # 0 = target, 1 = observation, 2 = not defined
