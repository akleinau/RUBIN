...
reader = BIFReader("endomcancerlast.bif")
model = reader.get_model()


# print(model.get_markov_blanket("Therapy"))
...
def add_markov_children(root, network, evidences=None, subsetFunc=None):
    blanket = model.get_markov_blanket(root.name)
...
add_markov_children(Therapy_Node, model, evidences=evidences, subsetFunc=find_changed_set)  #, subsetFunc=find_smallest_set)
deleteUseless(Therapy_Node, model, evidences)

Therapy_Node.print_tree()
DotExporter(Therapy_Node).to_picture("4changed.png")

# inference
infer = inference.VariableElimination(model)
Therapy_dist = infer.map_query(["Therapy"], evidence=evidences)
print(Therapy_dist)
